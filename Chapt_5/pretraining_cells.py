...
# %% Build initial (untrained) model and tokenizer
...
# %% Sanity check: generate from untrained model
...
# %% Manual loss walkthrough (probabilities of target tokens)
...
# %% Manual loss walkthrough (log-probas, negative average -> cross-entropy by hand)
#BACKPROPAGATION
...
# %% Load and split the-verdict.txt
...
# %% Pick device and compute initial loss (pre-training)
...
# %% Train the model (this is the slow cell — run once per fresh model)
torch.manual_seed(123)
model = GPTModel(GPT_CONFIG_124M)
model.to(device)
...
# %% Sampling demo: tiny vocab and logits
vocab = { 
    "closer": 0,
    "every": 1, 
    "effort": 2, 
...
# %% Temperature scaling + comparison plot
...
# %% Sample from each temperature distribution
...
def generate(model, idx, max_new_tokens, context_size, temperature=0.0, top_k=None, eos_id=None):
    for _ in range(max_new_tokens):         #1  The for loop is the same as before: gets logits and only focuses on the last
...
# %% loading model weights and setting to eval for inference
model = GPTModel(GPT_CONFIG_124M)
model.load_state_dict(torch.load("model.pth", map_location=device))
model.eval()
# %% saving weights and optimizer
torch.save({
